...
public class fairphoto {
	static HashMap<node, Integer>[] constant;
	static int least;
	static int[][] info;
...
//		System.out.println(System.currentTimeMillis() - start);
	...
	static int query(LinkedList<Integer> imp, int[] cnt, int poss) {
		...
		while (!flex.isEmpty()) {
			node key = generate(cnt, fixed, hash);
			int a = flex.poll();
	...
	static void update(LinkedList<Integer> sort, int[] cnt, int poss) {
		...
		if (constant[hash] == null)
			constant[hash] = new HashMap<node, Integer>();
	...
	static node generate(int[] cnt, boolean[] fixed, int hash) {
	...
	static class node {
		...
		public int hashCode() {
			...
			for (int i = 0; i < 8; i++) {
				hash += state[i];
				hash = (hash * 100001) % (Integer.MAX_VALUE );
			}
			return hash;
		}
