...
public class expert {
	...
	public static void main(String[] args) throws Exception {
		...
		int flagged = 0;
		boolean change = true;
		while (change && flagged != 99) {
			change = false;
			BufferedImage game = robot
					.createScreenCapture(new Rectangle(0, 0, (int) dim.getWidth(), (int) dim.getHeight()));
			
			int[][] gameBoard = new int[16][30];
			for (int i = 0; i < 16; i++) {
				for (int k = 0; k < 30; k++) {
					BufferedImage temp = game.getSubimage(541 + 16 * k, 184 + 16 * i, 16, 16);
					int z = 0;
					for (; z < 10; z++) {
					...
					if (1 <= gameBoard[i][k] && gameBoard[i][k] <= 8) {
						...
						for (int iC = -1; iC <= 1; iC++) {
							for (int kC = -1; kC <= 1; kC++) {
								if ((0 <= i + iC && i + iC < 16) && (0 <= k + kC && k + kC < 30)) {
								...
								for (int kC = -1; kC <= 1; kC++) {
									if ((0 <= i + iC && i + iC < 16) && (0 <= k + kC && k + kC < 30)) {
										if (gameBoard[i + iC][k + kC] == 0) {
											...
											robot.mouseMove(541 + 16 * (k + kC) + 8, 183 + 16 * (i + iC) + 8);
