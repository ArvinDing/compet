...
public class newbarns {
	...
	public static void main(String[] args) throws Exception {
		BufferedReader in = new BufferedReader(new FileReader("newbarns.in"));
		PrintWriter out = new PrintWriter(new File("newbarns.out"));
		StringTokenizer read = new StringTokenizer(in.readLine());
		n = Integer.parseInt(read.nextToken());
		LinkedList<Integer>[] adjancentList = new LinkedList[n];
		for (int i = 0; i < n - 1; i++) {
			read = new StringTokenizer(in.readLine());
			if(read.nextToken().equals("B")) {
				int prev=Integer.parseInt(read.nextToken());
